...
function getDecorationOptions() {
    ...
    const color = isLightTheme
        ? 'rgba(180, 141, 0, 0.33)' // Darker gold for light theme
        : 'rgba(253, 251, 251, 0.43)'; // Light color for dark theme
    return {
...
function isValuesFile(fileName) {
...
function updateDecorations(editor) {
    ...
    try {
        const valuesData = (0, yaml_1.parse)(text);
...
function findTemplateFiles(valuesPath) {
...
function findUsedValues(templateFiles) {
    const used = new Set();
...
function findUnusedValues(document, valuesData, usedValues) {
    const ranges = [];
    const lines = document.getText().split('\n');
    function checkObject(obj, prefix = '', startLine = 0) {
...
function isValueUsed(key, usedValues) {
